...
public class Book {
    ...
    public void editBook(int _id,String _name,Integer _author_id,Integer _genre_id,Integer _quantity,String _publisher,double _price,String _date_received,String _description,byte[] _cover){
        String updateQuery ;
       ...
       String query = "SELECT * FROM `books` WHERE `id`="+_id+" or `isbn`='"+_isbn+"'";  
        ...
        try {
            if(rs.next()){
                ...
                rs.getInt(5),rs.getInt(6),rs.getString(7),rs.getDouble(8),rs.getString(9),rs.getString(10),
                rs.getBytes(11));
    ...
    public void displayBooksCover(JLabel[] labels_tab){
        ...
        try {
             ...
             rs = st.executeQuery("SELECT `cover_image` FROM `books` order by id desc LIMIT 5");// desc
              byte[] image;
              int i = 0; // to pobulate the labels_tab items with image
